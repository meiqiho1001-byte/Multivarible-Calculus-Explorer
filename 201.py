...
with t_col2:
    ...
    is_elementary = all(isinstance(node, (sp.Add, sp.Mul, sp.Pow, sp.Symbol, sp.Number)) or any(isinstance(node, f) for f in elementary_funcs) for node in sp.preorder_traversal(f_sym))
...
try:
    ...
    if sols:
        for s in sols[:3]:
            ...
            st.write(f"📍 **Point ({s[x]}, {s[y]})** — **{label}**")
    else:
        st.info("No stationary points found in the current calculation.")
except:
